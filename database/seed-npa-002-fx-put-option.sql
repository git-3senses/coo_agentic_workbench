...
('NPA-2026-002','problem_statement','DBS institutional clients require downside hedging for GBP/USD currency risk. Existing product range covers only GBP/USD spot and forward. Clients holding GBP-denominated assets are exposed to GBP depreciation with no in-house option solution, leading to flow leakage to Barclays and HSBC.','MANUAL',100.00),
('NPA-2026-002','value_proposition','DBS offers institutional-grade GBP/USD put options providing precise downside hedge with defined premium cost, allowing clients to retain GBP upside while capping downside. Expected to capture 30+ institutional clients currently routing this flow to competitors. Annual revenue uplift: $2.8M.','MANUAL',100.00),
('NPA-2026-002','customer_benefit','(1) Defined cost hedge against GBP depreciation, (2) Full GBP upside retained, (3) Flexible tenors from 1 week to 12 months, (4) DBS as onshore SG counterparty (reduced basis vs offshore dealers), (5) Integrated with existing GBP/USD FX franchise for seamless hedging.','MANUAL',100.00),
...
('NPA-2026-002','product_role','Principal — DBS sells put options to clients, hedges delta exposure via spot and forward positions internally','AUTO',95.00),
...
('NPA-2026-002','target_roi','ROAE 18-22% (unfunded derivative — capital is operational risk only). Hurdle: 15% DBS minimum.','MANUAL',100.00),
...
('NPA-2026-002','customer_geographic','Singapore (primary), Hong Kong, other APAC. No US clients (exempt from CFTC requirements as non-US product on DBS SG entity).','MANUAL',100.00),
('NPA-2026-002','distribution_channels','GFM Institutional Sales (voice execution), Electronic FX platform (DBS FX Now — future phase), Bloomberg FXGO RFQ system','MANUAL',100.00),
...
('NPA-2026-002','competitive_landscape','GBP/USD options market: dominated by Barclays, HSBC, JPM. DBS advantage: SG-onshore execution, strong GBP franchise from UK trade finance clients, existing vol infrastructure from USD/SGD options. DBS currently losing 100% of GBP option flow to offshore banks.','ADAPTED',85.00),
('NPA-2026-002','market_opportunity','GBP/USD options market: $50B+ daily global volume. APAC institutional segment (ex-Japan): est. $5B daily. DBS target share: 1% = $50M daily notional, generating $3-4M annual premium income.','ADAPTED',82.00),
...
('NPA-2026-002','booking_system','Murex MX.3 — FXD|OPT|EUR typology. Portfolio: DBSSG_GFM_FX_OPT. No new system build — extension of existing FX options module for GBP/USD pair.','MANUAL',100.00),
...
('NPA-2026-002','portfolio_allocation','DBSSG_GFM_FX_OPT — FX Options Trading Book. Delta hedged against DBSSG_GFM_FX_SPOT portfolio.','MANUAL',100.00),
...
('NPA-2026-002','system_integration','No new integrations. Existing: Murex → Bloomberg (vol surface), Murex → MarkitWire (confirmations), Murex → SWIFT (settlements), Murex → DBS risk systems (VaR, P&L reporting).','MANUAL',100.00),
...
('NPA-2026-002','risk_system','Murex Risk (delta, vega, gamma, theta). DBS internal VaR system (historical simulation, 500-day window). Bloomberg OVDV for vol surface management.','MANUAL',100.00),
('NPA-2026-002','reporting_system','DBS internal risk dashboard (Murex). Bloomberg PORT for portfolio analytics. Internal: daily P&L flash, weekly risk report to Head of FX Desk, monthly report to GFM COO.','MANUAL',100.00),
...
('NPA-2026-002','security_assessment','N/A — No new systems. Existing Murex and Bloomberg infrastructure fully assessed and compliant under DBS ISS framework.','MANUAL',100.00),
...
('NPA-2026-002','continuity_measures','Existing: (1) Murex active-active Singapore/DR replication, (2) Bloomberg backup feeds (BVAL + ICE), (3) ICAP voice backup for forward points, (4) DBS Treasury backup FX platform for client-facing confirmations.','MANUAL',100.00),
...
('NPA-2026-002','xva_treatment','CVA: Applied for counterparties without daily CSA. DVA: Not applied per DBS policy. FVA: Applied for net cash outflows from delta hedges. All XVA computed by Middle Office using Murex XVA engine.','MANUAL',100.00),

-- ================================================================
-- PC.IV: RISK ANALYSIS
-- ================================================================
('NPA-2026-002','risk_classification','MEDIUM','AUTO',95.00),
('NPA-2026-002','market_risk','Primary risks: (1) Delta: GBP/USD spot exposure from option delta. Hedged daily via spot/forward. Residual delta: <GBP 5M. (2) Vega: GBP/USD vol exposure. Managed within vol book with Bloomberg BVAL hedges. (3) Gamma: short gamma position for DBS (client buys puts, DBS sells). Gamma risk most significant on expiry date. (4) Theta: positive theta (time decay benefits DBS as option seller). Residual market risk within standard FX desk limits.','MANUAL',100.00),
...
('NPA-2026-002','stress_scenarios','Scenario 1: GBP flash crash -15% (2016 Brexit replay) — large in-the-money position, clients exercise, DBS pays USD P&L (mitigated by delta hedge). Scenario 2: Vol spike to 25% (from 8.5% ATM) — mark-to-market loss on short vega position. Scenario 3: Simultaneous GBP crash + vol spike — maximum combined loss $8M on current book size. Within DBS stress loss limit for FX desk.','MANUAL',100.00),
('NPA-2026-002','stress_test_results','Back-test on 2016 GBP/USD crash: DBS short vega position loss = $2.3M at current book size. Acceptable within $10M FX desk stress buffer. Brexit scenario: residual delta risk fully mitigated by spot hedge. All stress scenarios within approved risk appetite.','MANUAL',100.00),
...
('NPA-2026-002','licensing_requirements','No new licensing. DBS holds MAS CMS licence covering FX derivatives. GBP/USD options within existing licence scope.','MANUAL',100.00),
...
('NPA-2026-002','tax_jurisdictions','SG: GST Exempt. UK: No WHT. No other jurisdictions unless client specific. Transfer pricing: N/A (DBS SG-only booking).','MANUAL',100.00),
...
('NPA-2026-002','data_ownership','DBS Bank Ltd Singapore. Trade data: Murex golden source. Client data: existing ISDA/KYC records.','MANUAL',100.00),
...
('NPA-2026-002','data_retention','Trade records: 7 years (MAS SFA requirement). Client communications: 5 years. Confirmation records: 7 years (ISDA requirement). Automated archival to DBSSG cold storage after 2 years.','MANUAL',100.00),
...
('NPA-2026-002','data_classification','BUSINESS_CONFIDENTIAL — Standard classification for GFM wholesale trading data. No sensitive personal data. Encryption per DBS data security standards (AES-256 at rest, TLS 1.3 in transit).','MANUAL',100.00),
...
-- ================================================================
('NPA-2026-002','booking_entity','DBS Bank Ltd, Singapore','MANUAL',100.00),
('NPA-2026-002','sales_entity','DBS Bank Ltd, Singapore (GFM Institutional Sales)','MANUAL',100.00),
...
('NPA-2026-002','risk_taking_entity','DBS Bank Ltd, Singapore (FX Trading Desk)','MANUAL',100.00),
('NPA-2026-002','risk_taking_location','Singapore','MANUAL',100.00),
('NPA-2026-002','processing_entity','DBS Bank Ltd, Singapore (GFM FX Operations)','MANUAL',100.00),
...
('NPA-2026-002','hedge_entity','DBS Bank Ltd, Singapore (FX Spot/Forward Book)','MANUAL',100.00),

-- ================================================================
-- APP.2: INTELLECTUAL PROPERTY
-- ================================================================
('NPA-2026-002','dbs_ip_exists','No','AUTO',100.00),
...
('NPA-2026-002','bribery_corruption','LOW — institutional wholesale product. No government procurement. Standard DBS anti-bribery and gifts/entertainment policy applies to all FX desk staff.','MANUAL',100.00),
...
-- ================================================================
('NPA-2026-002','rda_compliance','GBP/USD FX options data integrated into existing DBS risk data aggregation framework. Murex is golden source. No new BCBS 239 impact — same data model as existing FX options.','MANUAL',95.00),
...
('NPA-2026-002','app5_hedge_purpose','No — DBS sells options to clients and delta-hedges (not a designated hedge relationship under IFRS 9)','MANUAL',100.00),
('NPA-2026-002','collateral_types','CSA eligible: Cash (USD/GBP, 0% haircut), G7 Govt Bonds (2-5% haircut). DBS does not accept equities as collateral for FX options.','MANUAL',100.00),
...
('NPA-2026-002','booking_schema','Murex FXD|OPT|EUR|PUT. Single-leg option. Delta hedge: separate FX spot/forward booking in DBSSG_GFM_FX_SPOT. P&L aggregation: option book + delta hedge book combined for desk P&L reporting.','MANUAL',100.00),
