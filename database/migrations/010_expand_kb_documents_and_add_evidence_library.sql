-- Migration 010: Extend kb_documents for UI + add evidence_library (Feb 23, 2026)
-- Goal:
--   - Keep MCP tools using kb_documents as the single KB registry
--   - Add UI metadata columns to kb_documents (idempotent)
--   - Create evidence_library table (idempotent)
--   - Seed UI Knowledge Base + Evidence Library rows (upsert)
--
-- Compatible with MySQL and MariaDB (uses information_schema + dynamic SQL).

-- ────────────────────────────────────────────────────────────
-- 1) Extend kb_documents with UI metadata columns (if missing)
-- ────────────────────────────────────────────────────────────
...
-- Optional timestamps for UI sorting / auditing
SET @stmt := IF(
...
-- ────────────────────────────────────────────────────────────
-- 3) Seed: Knowledge Base (stored in kb_documents)
-- ────────────────────────────────────────────────────────────
