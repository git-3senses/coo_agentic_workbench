    ...
    <div class="h-full flex flex-col bg-gray-50 font-sans text-gray-900">
      ...
      <div class="flex-none bg-white border-b border-gray-200 px-8 py-5 flex items-center justify-between shadow-sm">
        <div>
          ...
          <p class="text-sm text-gray-500 mt-1">Pre-approved products with same-day trading under approved limits.</p>
        </div>
      ...
      <div class="flex-none bg-white border-b border-gray-200 px-8 py-4">
        <div class="grid grid-cols-4 gap-4 max-w-4xl">
          <div class="flex items-center gap-3 px-4 py-3 bg-gray-50 rounded-lg border border-gray-100">
            ...
            <div>
              <div class="text-xl font-bold text-gray-900">{{ products.length }}</div>
              <div class="text-[10px] font-bold text-gray-400 uppercase">Total Active</div>
            </div>
          </div>
          <div class="flex items-center gap-3 px-4 py-3 bg-gray-50 rounded-lg border border-gray-100">
            ...
            <div>
              <div class="text-xl font-bold text-gray-900">{{ avgUtilization }}%</div>
              <div class="text-[10px] font-bold text-gray-400 uppercase">Avg Utilization</div>
            </div>
          </div>
          <div class="flex items-center gap-3 px-4 py-3 bg-gray-50 rounded-lg border border-gray-100">
            ...
            <div>
              <div class="text-xl font-bold text-gray-900">{{ highUtilCount }}</div>
              <div class="text-[10px] font-bold text-gray-400 uppercase">&gt;80% Utilized</div>
            </div>
          </div>
          <div class="flex items-center gap-3 px-4 py-3 bg-gray-50 rounded-lg border border-gray-100">
            ...
            <div>
              <div class="text-xl font-bold text-gray-900">{{ breachedCount }}</div>
              <div class="text-[10px] font-bold text-gray-400 uppercase">Limit Breaches</div>
            </div>
          </div>
        </div>
      </div>
              ...
              <tbody class="divide-y divide-gray-100">
                <tr *ngFor="let p of products" class="hover:bg-gray-50 transition-colors">
                  <td class="px-6 py-4">
                    ...
                    <div class="text-[10px] text-gray-400 font-mono mt-0.5">{{ p.id }}</div>
                  </td>
                  <td class="px-6 py-4 text-gray-600">{{ p.product_category || 'N/A' }}</td>
                  ...
                  <td class="px-6 py-4 text-gray-400 text-[11px]">{{ formatDate(p.last_metric_date) }}</td>
                </tr>
              </tbody>
            </table>

            <div *ngIf="products.length === 0" class="text-center py-20">
              <lucide-icon name="refresh-cw" class="w-12 h-12 text-gray-300 mx-auto mb-4"></lucide-icon>
              <h3 class="text-lg font-medium text-gray-900">No Evergreen products</h3>
              <p class="text-gray-500 mt-2">No products are currently on the Evergreen track.</p>
            </div>
          </div>
        </div>
      </div>
