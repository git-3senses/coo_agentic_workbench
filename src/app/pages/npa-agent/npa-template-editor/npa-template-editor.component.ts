   ...
   template: `
    ...
    <div class="fixed inset-0 z-[200] bg-white flex flex-col font-sans animate-in fade-in duration-200">
      ...
      <div class="h-12 bg-slate-900 flex items-center justify-between px-4 z-20 flex-none">
        <div class="flex items-center gap-3">
          ...
          <div class="flex items-center gap-2.5">
             ...
             <div>
                <h1 class="text-[13px] font-semibold text-white leading-none">{{ getDocTitle() }}</h1>
                <p class="text-[10px] text-slate-400 mt-0.5">Full NPA &middot; Draft &middot; v1.0</p>
             </div>
          </div>
        </div>
        ...
        <div class="flex items-center gap-2">
           ...
           <div class="flex items-center bg-slate-800 rounded p-0.5">
              <button (click)="viewMode = 'document'"
                      ...
                      class="px-2.5 py-1 text-[11px] font-medium rounded transition-all text-slate-400 hover:text-slate-200">Doc</button>
              <button (click)="viewMode = 'form'"
                      [class.bg-slate-600]="viewMode === 'form'" [class.text-white]="viewMode === 'form'"
                      class="px-2.5 py-1 text-[11px] font-medium rounded transition-all text-slate-400 hover:text-slate-200">Form</button>
           </div>

           <!-- Lineage legend -->
           <div class="hidden lg:flex items-center gap-2.5 text-[10px] font-medium border-l border-slate-700 pl-2.5 ml-1">
           ...
           <button (click)="validateGovernance()" class="px-2.5 py-1 text-[11px] font-semibold text-slate-300 hover:text-white hover:bg-slate-700 rounded transition-colors flex items-center gap-1">
              <lucide-icon name="shield-check" class="w-3 h-3"></lucide-icon> Validate
           </button>
           <button (click)="save()" class="px-3 py-1 text-[11px] font-semibold text-white bg-blue-600 hover:bg-blue-700 rounded transition-colors flex items-center gap-1">
              <lucide-icon name="save" class="w-3 h-3"></lucide-icon> Save & Close
           </button>
        </div>
      </div>
      ...
      <div class="flex-1 overflow-hidden flex min-h-0">
         ...
         <div class="w-60 bg-slate-50 border-r border-gray-200 overflow-y-auto hidden lg:flex flex-col flex-none">

            <!-- Completion header -->
            <div class="px-4 py-3 border-b border-gray-200 bg-white">
               <div class="flex items-center justify-between mb-1.5">
                  <span class="text-[10px] font-bold text-gray-500 uppercase tracking-wider">Completion</span>
            ...
            <nav class="flex-1 py-1 overflow-y-auto">
               <a *ngFor="let section of sections; let i = index"
                  ...
                  class="group flex items-center gap-2 px-3 py-2 text-[11px] transition-all border-l-2 mx-1 rounded-r-md"
                  ...
                  <span class="font-mono text-[9px] text-gray-400 w-4 text-right flex-none">{{ getSectionNumber(i) }}</span>
            ...
            <!-- Field stats footer -->
            <div class="px-4 py-3 border-t border-gray-200 bg-white text-[10px] text-gray-500 space-y-1">
         ...
         <div class="flex-1 overflow-y-auto scroll-smooth relative min-h-0" id="form-container" (scroll)="onScroll($event)">
            ...
            <div class="bg-gradient-to-r from-slate-800 to-slate-900 px-8 py-4 text-white">
               <div class="flex items-center justify-between">
                  <div>
                     <p class="text-[10px] font-bold uppercase tracking-[0.15em] text-slate-400 mb-1">New Product Approval</p>
                  ...
                  <div class="text-right flex-none ml-6">
                     <div class="px-2.5 py-1 rounded bg-white/10 border border-white/20 inline-block">
                        <span class="text-[9px] font-bold text-slate-300 uppercase tracking-wider">Classification</span>
                        <p class="text-xs font-bold text-white">Full NPA</p>
                     </div>
                  </div>
               </div>
               <div class="flex items-center gap-5 mt-3 pt-2.5 border-t border-white/10 text-[10px] text-slate-400">
            ...
            <div class="bg-white">
               <div *ngFor="let section of sections; let si = index" [id]="'sec-' + section.id" class="border-b border-gray-100">
                  ...
                  <div class="sticky top-0 z-10 bg-gray-50/95 backdrop-blur-sm border-b border-gray-200 px-8 py-2.5 flex items-center gap-3">
                     ...
                     <h2 class="text-[13px] font-bold text-gray-900 flex-1">{{ section.title }}</h2>
                  ...
                  <div class="px-8 py-4 space-y-1">
                     <ng-container *ngFor="let field of section.fields">
                        ...
                        <!-- Header Field -->
                        <div *ngIf="field.type === 'header'" class="pt-4 pb-1">
                           <h3 class="text-[11px] font-bold text-gray-800 uppercase tracking-wider flex items-center gap-2">
                        ...
                        <div *ngIf="field.type === 'textarea'" class="py-2 group" (click)="onFieldFocus(field)">
                           <div class="flex items-center gap-2 mb-1">
                              <h4 class="text-[10px] font-bold text-gray-400 uppercase tracking-wide">{{ field.label }}</h4>
                              <span *ngIf="field.required" class="text-red-400 text-[9px]">REQ</span>
                           ...
                           <div *ngIf="editingField !== field.key"
                                class="text-[13px] text-gray-700 leading-relaxed whitespace-pre-wrap cursor-text rounded px-3 py-2 -mx-3 hover:bg-blue-50/40 transition-colors border border-transparent hover:border-blue-100"
                           ...
                           <textarea *ngIf="editingField === field.key"
                                     ...
                                     class="w-full text-[13px] text-gray-700 leading-relaxed rounded px-3 py-2 -mx-3 border border-blue-300 bg-blue-50/20 outline-none ring-2 ring-blue-100 resize-none"
                                     rows="4"></textarea>
                        </div>

                        <!-- Short fields — inline key:value -->
                        <div *ngIf="field.type !== 'textarea' && field.type !== 'header' && field.type !== 'file'"
                             class="group flex items-baseline gap-3 py-1.5 hover:bg-blue-50/30 -mx-3 px-3 rounded transition-colors cursor-text"
                             (click)="onFieldFocus(field)">
                           <span class="text-[12px] text-gray-500 flex-none truncate" style="width: 180px;">
                              {{ field.label }}<span *ngIf="field.required" class="text-red-400 text-[9px] ml-0.5">*</span>
                           </span>
                           <div class="flex-1 flex items-center gap-1.5 min-w-0">
                              <ng-container *ngIf="editingField !== field.key">
                                 <span class="text-[13px] leading-relaxed cursor-text truncate"
            ...
            <div class="max-w-3xl mx-auto py-6 space-y-6 pb-24 px-6">
               <div *ngFor="let section of sections" [id]="'sec-' + section.id" class="bg-white rounded-lg border border-gray-200 overflow-hidden">
                  <div class="px-5 py-3 border-b border-gray-100 bg-gray-50/50">
                     ...
                     <p *ngIf="section.description" class="text-xs text-gray-500 mt-0.5">{{ section.description }}</p>
                  </div>
                  <div class="p-5 grid grid-cols-1 md:grid-cols-2 gap-x-5 gap-y-4">
                     <div *ngFor="let field of section.fields" [class.md:col-span-2]="field.type === 'textarea' || field.type === 'header'" class="relative">
                        ...
                        <ng-container *ngIf="field.type !== 'header'">
                           <label class="block text-[10px] font-semibold text-gray-600 uppercase tracking-wide mb-1">
                           ...
                           <div class="relative" (click)="onFieldFocus(field)">
                              <select *ngIf="field.type === 'select'" [(ngModel)]="field.value"
                                      ...
                                      class="w-full text-sm rounded px-3 py-1.5 outline-none transition-all appearance-none bg-white">
                                 <option value="" disabled>Select...</option>
                                 <option *ngFor="let opt of field.options" [value]="opt">{{ opt }}</option>
                              </select>
                              <input *ngIf="field.type === 'date'" [(ngModel)]="field.value" type="date"
                                     [ngClass]="getInputStyles(field.lineage, focusedField?.key === field.key)"
                                     class="w-full text-sm rounded px-3 py-1.5 outline-none transition-all">
                              <input *ngIf="field.type !== 'textarea' && field.type !== 'file' && field.type !== 'select' && field.type !== 'date'"
                                     ...
                                     [ngClass]="getInputStyles(field.lineage, focusedField?.key === field.key)"
                                     class="w-full text-sm rounded px-3 py-1.5 outline-none transition-all">
                              <textarea *ngIf="field.type === 'textarea'" [(ngModel)]="field.value" rows="3"
                                        [placeholder]="field.placeholder || ''"
                                        [ngClass]="getInputStyles(field.lineage, focusedField?.key === field.key)"
                                        class="w-full text-sm rounded px-3 py-1.5 outline-none transition-all"></textarea>
                           </div>
                        </ng-container>
                     </div>
                  </div>
         ...
         <div class="w-72 bg-white border-l border-gray-200 flex-none flex flex-col overflow-y-auto hidden lg:flex">
            ...
            <ng-container *ngIf="focusedField">
               <div class="h-10 border-b border-gray-100 flex items-center justify-between px-3 bg-gray-50 flex-none">
                   <h3 class="text-[10px] font-bold uppercase tracking-wider text-gray-500 flex items-center gap-1">
                      <lucide-icon name="eye" class="w-3 h-3"></lucide-icon> Source Inspector
                   </h3>
               ...
               <div class="p-3 flex-1 overflow-y-auto space-y-4">
                   <div>
                      <p class="text-[9px] uppercase font-bold text-gray-400 mb-0.5">Field</p>
                      <h2 class="text-xs font-bold text-gray-900">{{ focusedField.label }}</h2>
                      <div class="mt-1.5 flex items-center gap-2">
                         ...
                         <span *ngIf="focusedField.lineageMetadata?.confidenceScore" class="text-[9px] font-medium text-gray-400">
                   ...
                   <div *ngIf="focusedField.lineage !== 'MANUAL'">
                      <div *ngIf="focusedField.lineageMetadata?.adaptationLogic" class="bg-amber-50 rounded-md p-2.5 border border-amber-100 mb-3">
                         <p class="text-[9px] font-bold text-amber-800 mb-0.5">Adaptation Logic</p>
                         <p class="text-[10px] text-amber-700 leading-relaxed">{{ focusedField.lineageMetadata?.adaptationLogic }}</p>
                      </div>
                      <div *ngIf="focusedField.lineageMetadata?.sourceSnippet">
                         <p class="text-[9px] uppercase font-bold text-gray-400 mb-0.5">Source</p>
                         <div class="bg-slate-50 rounded-md p-2 border border-slate-200 text-[10px] text-gray-600 leading-relaxed">
                            "{{ focusedField.lineageMetadata?.sourceSnippet }}"
                         </div>
                      </div>
                      <button class="w-full mt-3 py-1.5 bg-white border border-gray-200 text-gray-500 hover:text-blue-600 hover:border-blue-300 rounded text-[10px] font-semibold transition-colors flex items-center justify-center gap-1">
                   ...
                   <div *ngIf="focusedField.lineage === 'MANUAL'">
                      <div class="bg-blue-50 rounded-md p-3 border border-blue-100 text-center">
                         ...
                         <h4 class="text-[11px] font-bold text-blue-900 mb-0.5">Need help?</h4>
                         <p class="text-[9px] text-blue-700 mb-2">{{ focusedField.lineageMetadata?.agentTip || 'I can search KB for similar clauses.' }}</p>
                         <button class="w-full py-1.5 bg-blue-600 hover:bg-blue-700 text-white rounded text-[10px] font-bold transition-colors">
                            Ask Agent to Draft
                         </button>
                      </div>
                   </div>
            ...
            <ng-container *ngIf="!focusedField">
               <div class="h-10 border-b border-gray-100 flex items-center px-3 bg-gray-50 flex-none">
                   <h3 class="text-[10px] font-bold uppercase tracking-wider text-gray-500 flex items-center gap-1">
                      <lucide-icon name="bar-chart-3" class="w-3 h-3"></lucide-icon> Document Summary
                   </h3>
               </div>
               ...
               <div class="p-3 flex-1 overflow-y-auto space-y-4">
                  ...
                  <div class="text-center py-3">
                     ...
                     <p class="text-[10px] text-gray-400 font-medium mt-0.5">Overall Completion</p>
                  </div>

                  <!-- Lineage breakdown -->
                  <div>
                     <p class="text-[9px] font-bold text-gray-400 uppercase tracking-wider mb-2">Lineage Breakdown</p>
                     <div class="space-y-2">
                        <div class="flex items-center gap-2">
                           <span class="w-2 h-2 rounded-full bg-emerald-500 flex-none"></span>
                           <span class="text-[11px] text-gray-600 flex-1">Auto-filled</span>
                           <span class="text-[11px] font-bold text-gray-800">{{ getLineageCount('AUTO') }}</span>
                        </div>
                        <div class="flex items-center gap-2">
                           <span class="w-2 h-2 rounded-full bg-amber-500 flex-none"></span>
                           <span class="text-[11px] text-gray-600 flex-1">Adapted</span>
                           <span class="text-[11px] font-bold text-gray-800">{{ getLineageCount('ADAPTED') }}</span>
                        </div>
                        <div class="flex items-center gap-2">
                           <span class="w-2 h-2 rounded-full bg-red-500 flex-none"></span>
                           <span class="text-[11px] text-gray-600 flex-1">Manual</span>
                           <span class="text-[11px] font-bold text-gray-800">{{ getLineageCount('MANUAL') }}</span>
                        </div>
                        <div class="flex items-center gap-2 pt-1 border-t border-gray-100">
                           <span class="w-2 h-2 rounded-full bg-gray-300 flex-none"></span>
                           <span class="text-[11px] text-gray-600 flex-1">Empty</span>
                           <span class="text-[11px] font-bold text-gray-400">{{ getTotalFieldCount() - getFilledFieldCount() }}</span>
                        </div>
                     </div>
                     <!-- Stacked bar -->
                  ...
                  <div>
                     <p class="text-[9px] font-bold text-gray-400 uppercase tracking-wider mb-2">Section Status</p>
                     <div class="space-y-1.5">
                        <div *ngFor="let section of sections; let i = index" class="flex items-center gap-2 text-[10px]">
                           ...
                           <span class="font-mono font-bold text-[9px]"
                  ...
                  <div class="pt-2 border-t border-gray-100 space-y-2">
                     <p class="text-[9px] font-bold text-gray-400 uppercase tracking-wider mb-1">Actions</p>
                     <button (click)="validateGovernance()" class="w-full py-1.5 bg-slate-50 hover:bg-slate-100 border border-gray-200 rounded text-[10px] font-semibold text-gray-700 transition-colors flex items-center justify-center gap-1.5">
                        <lucide-icon name="shield-check" class="w-3 h-3"></lucide-icon> Run Governance Check
                     </button>
                     <button class="w-full py-1.5 bg-slate-50 hover:bg-slate-100 border border-gray-200 rounded text-[10px] font-semibold text-gray-700 transition-colors flex items-center justify-center gap-1.5">
                        <lucide-icon name="wand-2" class="w-3 h-3"></lucide-icon> Auto-fill Empty Fields
                     </button>
                  </div>

                  <!-- Tip -->
                  <div class="bg-blue-50 rounded-md p-2.5 border border-blue-100 mt-2">
                     <p class="text-[10px] text-blue-700 leading-relaxed"><strong>Tip:</strong> Click any field to see its source lineage, adaptation logic, and confidence score in this panel.</p>
                  </div>
               </div>
            </ng-container>
         </div>
