...
@Component({
    ...
    <div class="h-full flex flex-col bg-gray-50 font-sans text-gray-900">
      ...
      <!-- HEADER -->
      <div class="flex-none bg-white border-b border-gray-200 px-8 py-5 shadow-sm">
        <h1 class="text-2xl font-bold text-gray-900 tracking-tight flex items-center gap-3">
          <lucide-icon name="package" class="w-6 h-6 text-blue-600"></lucide-icon>
          Bundling Assessment
        </h1>
        <p class="text-sm text-gray-500 mt-1">Evaluate whether a new product can be bundled under an existing approved product.</p>
      </div>
        ...
        <div class="max-w-3xl mx-auto space-y-8">
          ...
          <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-6 space-y-4">
            ...
            <div class="grid grid-cols-2 gap-4">
              <div>
                <label class="block text-xs font-bold text-gray-500 uppercase mb-1.5">Child NPA (New Product)</label>
                <select [(ngModel)]="childId" class="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-200">
                  <option value="">Select NPA...</option>
                  <option *ngFor="let n of npaList" [value]="n.id">{{ n.id }} - {{ n.title }}</option>
                </select>
              </div>
              <div>
                <label class="block text-xs font-bold text-gray-500 uppercase mb-1.5">Parent NPA (Approved Product)</label>
                <select [(ngModel)]="parentId" class="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-200">
          ...
          <div *ngIf="assessment" class="space-y-6">
            ...
            <div class="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
              <div class="px-6 py-4 border-b border-gray-200">
                <h3 class="text-sm font-bold text-gray-900">Condition Details</h3>
              </div>
              <div class="divide-y divide-gray-100">
                <div *ngFor="let c of assessment.conditions; let i = index" class="px-6 py-4 flex items-start gap-4">
                  ...
                  <div class="flex-1">
                    <div class="text-sm font-semibold text-gray-900">{{ c.condition }}</div>
