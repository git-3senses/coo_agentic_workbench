...
## 1A. Baseline Performance Metrics (from Deep Knowledge Section 17)

These are the current-state baselines that the ML Prediction Agent uses as reference points for predictions and improvement targets:
...
**Usage Notes**: These baselines inform the ML model's predictions. When the model predicts a timeline, it contextualizes against these averages. For example, if the model predicts 8 days for an NPA that would typically take 12 days, it explains: "33% faster than baseline due to [factors]."
...
Negative Factors:
- High notional ($75M > $50M): Finance VP scrutiny (-12%)
- Cross-border booking: Added complexity (-12%)
- Q4 timing: Legal slower (year-end rush) (-8%)
- Approver workload: 2x typical load (-6%)
...
**Scenario**: User wants Bitcoin Options. DBS never traded crypto derivatives.
...
if classification == "NTG" and similar_product_count == 0:
  display_message = "This is New-to-Group. We have no historical NPAs for Bitcoin Options."
...
if notional > 3 * std_dev_from_training_mean:
  detect_outlier = True
  cap_prediction = True
  message = "Notional extremely high ($5B). Model predictions may be unreliable for deals this size."
...
if signal_conflict_detected:
  ...
  explanation = "Mixed signals detected. Product type strong (+25%), but desk track record weak (-18%). Prediction has higher uncertainty."
```
